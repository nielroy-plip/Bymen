...
export class HomologService {
  ...
  async registerUser(dto: RegisterUserDto) {
    ...
    if (!username || username.length < 3) {
      throw new BadRequestException('Usuário deve ter no mínimo 3 caracteres');
    }
  ...
  async upsertClient(dto: UpsertClientDto) {
    const documentRaw = (dto.cnpjCpf || '').trim();
    const normalizedDocument = documentRaw.replace(/\D/g, '');
    const documentHash = createHash('sha256').update(normalizedDocument || dto.id).digest('hex');
  ...
  async saveMeasurement(dto: SaveMeasurementDto) {
    const contentHash = createHash('sha256').update(JSON.stringify(dto)).digest('hex');
  ...
  private async ensureProduct(dto: StockMovementDto) {
